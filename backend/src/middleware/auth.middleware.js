...
/**
 * Refresh token validation and token regeneration
 */
const refreshTokenValidation = tryCatch(async (refreshToken, res, next) => {
  let refreshDecode;
  try {
  ...
  // Validate user by id from refresh token
  const user = await validateUserById(refreshDecode.id, next);
  ...
  try {
    ...
    try {
      ...
      if (err.message === 'Token has expired') {
        ...
        // If refresh token valid, get user and set req.user, then next()
